...
The model is thought of as being in three layers (from left to right): the visible layer, the hidden layer, and the output layer. The hidden layer activation nodes values <span>\[a_i^{(2)}, i =1,\dots,3\]<\span> are each calculated as the sigmoid function of the linear combination of <span>\[x_j\]<\span>'s. For example, <span>\[a_i^{(2)} = g\left(\sum_{j=1}^3\theta^{(1)}_{ij}x_j\right)\]<\span> where <span>\[g(x) = \frac{1}{1+e^{-x}}\]<\span>. Then, at the output layer, each of these activation units is put together the same way to get <span>\[h_\theta(x) = g\left(\sum_{j=1}^3\theta^{(2)}_{ij}a^{(2)}_j\right)\]<\span>. In this way, neural networks are similar to inception.
